...
<cfquery name="GET_SHIP_PACKAGE_LIST" datasource="#dsn1#">
        ...
        FROM
            ...
            FROM
                ...
                (SELECT
                 ...
                 WHERE
                 	(#dsn2_alias#.SHIP_ROW.SHIP_ID = #attributes.ship_id#) AND
	...
	FROM
        ...
        (SELECT
         ...
         WHERE
            ...
            (SHIP_ROW_3.SHIP_ID = #attributes.ship_id#) AND 
	...
	WHERE
        ...
        (SHIP_ROW_2.SHIP_ID = #attributes.ship_id#) AND
	...
	FROM
        ...
        STOCKS AS STOCKS_1 ON PRODUCT_2.PRODUCT_ID = STOCKS_1.PRODUCT_ID RIGHT OUTER JOIN
         ...
         WHERE
            ...
            (SHIP_ROW_1.SHIP_ID = #attributes.ship_id#)
       	 GROUP BY 
...
<form name="add_package" method="post" action="<cfoutput>#request.self#?fuseaction=pda.popup_add_ship_package&SHIP_ID=#URLDecode(URL.ship_id)#&department_id=#department_id#&date1=#date1#&date2=#date2#&page=#page#&kontrol_status=#kontrol_status#</cfoutput>">
	<tr class="color-list">
		<td colspan="4">
		<table cellpadding="2" cellspacing="1" width="98%">
			<tr height="20px"><td colspan="4"><b>İrsaliye No : <cfoutput>#URLDecode(URL.belge_no)#</cfoutput></b></td></tr>
			<tr>
				<td>Miktar</td>
				<td><strong><input name="add_other_amount" type="text" value="<cfoutput>#TlFormat(attributes.add_other_amount,0)#</cfoutput>" onKeyup="return(FormatCurrency(this,event,0));" class="moneybox" style="width:40px;"></strong></td>
	...
	<tr id="row#STOCK_ID#" height="20" onMouseOver="this.className='color-light';" onMouseOut="this.className='color-row';" class="color-row">
		<td>
            ...
            <td>
            <input type="text" name="control_amount#STOCK_ID#" value="<cfif isdefined('control_amount#STOCK_ID#')>#Tlformat(Evaluate('control_amount#STOCK_ID#'),0)# <cfelse>#Tlformat(0,0)#</cfif>" class="box"  style="width:25px;text-align:right;color:FF0000;">
