...
SELECT PP.SHELF_CODE,SL.COMMENT,D.DEPARTMENT_HEAD,PP.LOCATION_ID,PP.STORE_ID FROM #DSN3#.PRODUCT_PLACE_ROWS AS PPR 
	...
	LEFT JOIN #dsn#.STOCKS_LOCATION AS SL ON SL.LOCATION_ID=PP.LOCATION_ID AND SL.DEPARTMENT_ID=PP.STORE_ID
	LEFT JOIN #dsn#.DEPARTMENT AS D ON D.DEPARTMENT_ID=SL.DEPARTMENT_ID 
WHERE PPR.STOCK_ID=#attributes.sid_# AND D.BRANCH_ID IN (SELECT D.BRANCH_ID FROM #dsn#.EMPLOYEE_POSITIONS AS EP INNER JOIN #dsn#.DEPARTMENT AS D ON D.DEPARTMENT_ID =EP.DEPARTMENT_ID WHERE EP.POSITION_CODE=#session.ep.POSITION_CODE#)
</cfquery>
<cf_grid_list>
	<tr>
		<th>Raf</th>
