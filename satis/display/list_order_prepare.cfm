...
LEFT JOIN (
	select sum (SFR.AMOUNT) AS AMOUNT ,UNIQUE_RELATION_ID  FROM #DSN2#.STOCK_FIS_ROW AS SFR GROUP BY UNIQUE_RELATION_ID
  UNION ALL
select sum (SFR.AMOUNT) AS AMOUNT ,UNIQUE_RELATION_ID  FROM workcube_metosan_2024_1.STOCK_FIS_ROW AS SFR GROUP BY UNIQUE_RELATION_ID
UNION ALL
...
<cfquery name="GETSEVKS_2" datasource="#DSN3#">
 SELECT DISTINCT ISNULL(S.AMOUNT,0) AS AMOUNT,I.INTERNAL_NUMBER,I.DEPARTMENT_OUT,I.LOCATION_OUT,I.PROJECT_ID,workcube_metosan.getEmployeeWithId(EP.EMPLOYEE_ID) AS PERSONAL,PREPARE_PERSONAL,I.INTERNAL_ID,D.DEPARTMENT_HEAD,SL.COMMENT,I.DEPARTMENT_OUT,I.LOCATION_OUT FROM workcube_metosan_1.INTERNALDEMAND AS I
...
LEFT JOIN (	SELECT SUM(AMOUNT) AMOUNT,WRK_ROW_RELATION_ID FROM (
			SELECT AMOUNT,WRK_ROW_RELATION_ID FROM #DSN2#.SHIP_ROW AS S
			UNION ALL
			SELECT AMOUNT,WRK_ROW_RELATION_ID FROM workcube_metosan_2024_1.SHIP_ROW AS S 
      UNION ALL
