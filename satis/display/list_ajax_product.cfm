        ...
        <tr>
            ...
            <td>
                ...
                <cfif isDefined("attributes.actType") and len(attributes.actType)>
                    ...
                    <cfif attributes.actType eq 3>
                        <a onclick="addTreeItem_('#it.STOCK_CODE#','#it.PRODUCT_NAME#',#attributes.SIPARIS_MIKTARI#,#it.STOCK_ID#,#it.PRODUCT_ID#,'#it.MAIN_UNIT#',#it.STOCK_COUNT#,0,#attributes.columnsa#)">#it.PRODUCT_NAME#</a>
                    </cfif>
   ...
   <!--- <cfset arguments.keyword = Replace(arguments.keyword,' ',';','all')>----><!--- % idi ; yaptik --->
    ...
    <cfquery name="get_productsss" datasource="#arguments.dsn1#" result="getproducts_result">
        SELECT TOP 100
            ...
            ISNULL(GPA.MONEY, PRICE_STANDART.MONEY) MONEY,
            PRODUCT.TAX,
            (SELECT SUM(BAKIYE)
FROM (
	SELECT SUM(STOCK_IN - STOCK_OUT) AS BAKIYE
		,CONVERT(VARCHAR, STORE) + '-' + CONVERT(VARCHAR, STORE_LOCATION) AS DEPO
		,STOCK_ID
	FROM workcube_metosan_2023_1.STOCKS_ROW
	WHERE STOCK_ID = STOCKS.STOCK_ID
	GROUP BY STORE
		,STORE_LOCATION
		,STOCK_ID
	) AS T
