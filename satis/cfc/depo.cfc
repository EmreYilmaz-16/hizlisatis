...
<CFIF IIIX LT GETPERIOD.recordcount>
UNION ALL 
</CFIF>
<CFSET IIIX=IIIX+1>
</CFLOOP>
...
<!------	SELECT SUM(AMOUNT) AMOUNT,UNIQUE_RELATION_ID FROM workcube_metosan_2024_1.STOCK_FIS_ROW GROUP BY UNIQUE_RELATION_ID
	...
	UNION ALL
	SELECT SUM(AMOUNT) AMOUNT,UNIQUE_RELATION_ID FROM workcube_metosan_2023_1.STOCK_FIS_ROW GROUP BY UNIQUE_RELATION_ID
	UNION ALL
	SELECT SUM(AMOUNT) AMOUNT,UNIQUE_RELATION_ID FROM workcube_metosan_2022_1.STOCK_FIS_ROW GROUP BY UNIQUE_RELATION_ID
	) AS SF
	ON SF.UNIQUE_RELATION_ID=ORR.UNIQUE_RELATION_ID----->
          <cfquery name="getDepWorks" datasource="#dataSources.dsn#">
            SELECT DISTINCT  O.RECORD_DATE,
                ...
                LEFT JOIN #dataSources.dsn3#.PRTOTM_SHIP_RESULT AS SR ON SR.SHIP_RESULT_ID=SRR.SHIP_RESULT_ID
                LEFT JOIN (
                    SELECT SUM(AMOUNT) AMOUNT,UNIQUE_RELATION_ID FROM #dataSources.dsn#_#CURRENT_YEAR#_1.STOCK_FIS_ROW GROUP BY UNIQUE_RELATION_ID
                    UNION ALL
                    SELECT SUM(AMOUNT) AMOUNT,UNIQUE_RELATION_ID FROM #dataSources.dsn#_#PAST_YEAR#_1.STOCK_FIS_ROW GROUP BY UNIQUE_RELATION_ID
                    UNION ALL
                    SELECT SUM(AMOUNT) AMOUNT,UNIQUE_RELATION_ID FROM #dataSources.dsn#_#PAST_YEAR#_1.STOCK_FIS_ROW GROUP BY UNIQUE_RELATION_ID
                ) AS SF ON SF.UNIQUE_RELATION_ID=ORR.UNIQUE_RELATION_ID
                LEFT JOIN #dataSources.dsn3#.STOCKS AS S ON S.STOCK_ID=ORR.STOCK_ID
                LEFT JOIN #dataSources.dsn3#.PRODUCT_PLACE_ROWS AS PPR ON PPR.STOCK_ID=S.STOCK_ID
                LEFT JOIN #dataSources.dsn3#.PRODUCT_PLACE AS PP ON PP.PRODUCT_PLACE_ID=PPR.PRODUCT_PLACE_ID
                LEFT JOIN #dataSources.dsn#.COMPANY AS C ON C.COMPANY_ID=SR.COMPANY_ID
...
SELECT DISTINCT ISNULL(S.AMOUNT,0) AS AMOUNT,I.INTERNAL_NUMBER,I.DEPARTMENT_OUT,I.LOCATION_OUT,I.PROJECT_ID,workcube_metosan.getEmployeeWithId(EP.EMPLOYEE_ID) AS KAYDEDEN,I.INTERNAL_ID,DEPARTMENT_IN,LOCATION_IN FROM #datasources.dsn3#.INTERNALDEMAND AS I
...
LEFT JOIN #datasources.dsn#.EMPLOYEE_POSITIONS AS EP ON EP.POSITION_CODE=I.TO_POSITION_CODE
...
<cfloop query="getDemands">
    <cfscript> aWork={
        ...
        DELIVERT_DEPT=DEPARTMENT_IN,
        DELIVER_LOCATION=LOCATION_IN,
        NICKNAME="İç Talep",
...
WHERE FROM_PROJE=1
AND DEPARTMENT_OUT=#arguments.DEPARTMENT_ID# AND LOCATION_OUT=#arguments.LOCATION_ID# AND OCC=1 AND INTERNALDEMAND.PREPARE_PERSONAL IS NULL AND  INTERNALDEMAND.INTERNALDEMAND_STAGE <> 353
<CFSET returnArr=arrayNew(1)>
