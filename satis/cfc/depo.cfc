...
<cfset dsn=application.systemparam.dsn>
    ...
    <cffunction name="getDepartmentEmployees" httpMethod="POST" access="remote" returntype="any" returnFormat="json">
        ...
        SELECT WIDTH AS EMP_ID,DEPARTMENT_ID,LOCATION_ID FROM #dsn#.STOCKS_LOCATION
        UNION
        SELECT HEIGHT  AS EMP_ID,DEPARTMENT_ID,LOCATION_ID FROM #dsn#.STOCKS_LOCATION
        UNION 
        SELECT DEPTH  AS EMP_ID,DEPARTMENT_ID,LOCATION_ID FROM #dsn#.STOCKS_LOCATION
        ) AS T
...
,(SELECT COUNT(*) FROM workcube_metosan_1.ORDER_ROW where ORDER_ID =O.ORDER_ID AND DELIVER_DEPT=ORR.DELIVER_DEPT AND DELIVER_LOCATION=ORR.DELIVER_LOCATION) AS TTS
