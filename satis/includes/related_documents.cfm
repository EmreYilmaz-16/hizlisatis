...
<cf_box title="İlişkili Fatura ve İrsaliyeler">
   ...
   SELECT * FROM (
select OI.SHIP_ID as INVOICE_ID,SP.PERIOD_ID,SP.PERIOD_YEAR,OI.ORDER_ID,I.INVOICE_NUMBER,'SHIP' AS TIP,ACTION_DATE from workcube_metosan_1.ORDERS_SHIP AS OI LEFT JOIN workcube_metosan.SETUP_PERIOD AS SP ON SP.PERIOD_ID=OI.PERIOD_ID
...
select OI.INVOICE_ID,SP.PERIOD_ID,SP.PERIOD_YEAR,OI.ORDER_ID,I.INVOICE_NUMBER,'INVOICE' AS TIP,ACTION_DATE from workcube_metosan_1.ORDERS_INVOICE AS OI LEFT JOIN workcube_metosan.SETUP_PERIOD AS SP ON SP.PERIOD_ID=OI.PERIOD_ID
