function openNoteBox() {
    if ($('#company_id_').val())
        openBoxDraggable('index.cfm?fuseaction=objects.ajax_extra_notes_list&style=1&design_id=1&is_special=0&action_type=0&is_delete=1&action_section=COMPANY_ID&action_id=' + $('#company_id_').val() + '&is_open_det=1');
}
function openPayMethods() {
    if ($('#company_id_').val()) {
        ...
        openBoxDraggable('index.cfm?fuseaction=objects.popup_extra_paymethods' + url_param, "PayMethodModal");
    }
}
...
function fillFields() {
    if (!list_find("37,38,39,40", window.event.keyCode)) {
        ...
        if (compInfo.BAKIYE < 0)
            $("#bakiye").css("color", "red");
        else
            $("#bakiye").css("color", "green");
        ...
        compInfo.PRICE_LISTS.forEach((list, index) => {
            ...
            let newOption = "<option value='" + list.PRICE_CATID + "' " + selected_status + ">" + list.PRICE_CAT + "</option>";
...
function applyFilters(type) {
    if (!list_find("37,38,39,40", window.event.keyCode) && (window.event.keyCode == "13" || type > 0) && $('#company_id_').val() && $("#price_list").val()) {
        $("#product_list").text('');
        var url_params = "";
...
function openPriceList(type, pid_, sid_, tax, cost, manuel, product_name, stock_code, brand, indirim1, deliver_dept, basket_row_departman, deliver_date, amount, unit, price, other_money, price_other, product_name_other, detail_info_extra, row_nettotal) {
    ...
    openBoxDraggable('index.cfm?fuseaction=objects.popup_extra_product_prices' + price_params)
}
