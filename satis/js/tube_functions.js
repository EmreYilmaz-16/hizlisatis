...
function FindProduct2(ev, el, userid, dsn2, dsn1, dsn3, price_catid, comp_id) {
  ...
  if ((ev.keyCode == 13 || ev.type == "change") && keyword.length > 5) {
    ...
    if (Product.RECORDCOUNT != 0) {
      ...
      div.setAttribute(
        "style",
        "display:none;position: fixed;z-index: 99999; background: whitesmoke;padding: 17px;border: solid 1px #ad6d6d;"
      );
      ...
      for (let i = 0; i < Product.PRODUCTS.length; i++) {
        ...
        a.setAttribute(
          ...
          "setRwTube('" +
            ...
            "," +
            Product.PRODUCTS[i].STOCK_ID +
            "," +
            Product.PRODUCTS[i].DISCOUNT_RATE +
            "," +
            Product.PRODUCTS[i].PRICE +
            ",'" +
            Product.PRODUCTS[i].REL_CATNAME +
            "','" +
            Product.PRODUCTS[i].REL_HIERARCHY +
            "','" +
            Product.PRODUCTS[i].MONEY +
            "'," +
            Product.PRODUCTS[i].REL_CATID +
            ")"
        );
...
function CalculateTube() {
  ...
  var AdditionalProduct_DSC = document.getElementById(
    "AdditionalProduct_DSC"
  ).value;
  ...
  var AdditionalProduct_MNY = document.getElementById(
  ...
  AdditionalProduct_Qty = parseFloat(
    filterNum(commaSplit(AdditionalProduct_Qty))
  );
  AdditionalProduct_DSC = parseFloat(
    filterNum(commaSplit(AdditionalProduct_DSC))
  );
  var ax = DegerLeriHesapla(
    AdditionalProduct_Prc,
    AdditionalProduct_DSC,
    AdditionalProduct_Qty,
    AdditionalProduct_MNY
    ...
    if (name.trim().length > 0) {
      ...
      if (generalParamsSatis.workingParams.IS_SAME_VIRTUAL_NAME == 0) {
        while (true) {
          ...
          var r = wrk_query(
            "SELECT * FROM VIRTUAL_PRODUCTS_PRT WHERE PRODUCT_NAME='" +
              name +
              "'",
            "dsn3"
          );
...
function TubeControl() {
  ...
  var AdditionalProduct_PId = document.getElementById(
    "AdditionalProduct_PId"
  ).value;
  ...
  if (Q.IS_REQUIRED == 1) {
    if (
      parseInt(AdditionalProduct_PId || AdditionalProduct_PId.length == 0) == 0
    ) {
