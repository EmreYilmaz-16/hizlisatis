...
function setDoom() {
    ...
    $("#basketArea").find(".ui-scroll").attr("style", "height:" + ((window.innerHeight * 50) / 100) + "px;max")
...
function AddRow(
    ...
    if (product_unit == 'M' && fc == 0) {
        ...
        openBoxDraggable('index.cfm?fuseaction=objects.emptypopup_extra_calculate_pbs' + calculate_params);
        return true;
    }
    ...
    if (res.recordcount > 0) {
        if (res.recordcount > 1) {
            ...
            calculate_params += "&shelf_code=" + shelf_code;
            calculate_params += "&rowNum=" + row_count;
            openBoxDraggable('index.cfm?fuseaction=objects.emptypopup_select_raf_pbs' + calculate_params);
            return true;
        } else {
...
function rowaListener(tr) {
    $(tr).on("contextmenu", function (ev) {
        ...
        $(div).attr("style", "display:none;top:" + ev.clientY + "px;left:" + ev.clientX + "px")
...
function rowArrange() {
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        var product_name_other = document.getElementById("product_name_other_" + Old_rw_id);
        ...
        var detail_info_extra = document.getElementById("detail_info_extra_" + Old_rw_id);
...
function openPriceListPartner(PRODUCT_ID,STOCK_ID,MAIN_UNIT,company_id){
    ...
    var uri="objects.popup_product_price_history_js"+MoneyList+"&sepet_process_type=-1&product_id="+PRODUCT_ID+"&stock_id="+STOCK_ID+"&pid="+PRODUCT_ID+"&product_name=&unit="+MAIN_UNIT+"&row_id=0&company_id="+company_id;
