...
function saveVirtualTube(dsn3, modal_id) {
    ...
    $.ajax({
        ...
        success: function (retDat) {
            ...
            AddRow(obj.PID, '', 1, 1, obj.PRICE, obj.NAME, 20, 0, 1, '', "TL", obj.PRICE,"-5");
...
function AddRow(pid, sid, is_virtual, qty, price, p_name, tax, discount_rate, poduct_type = 0, shelf_code = '', omoney = 'TL', price_other, currency = "-6") {
    row_count++;
    rowCount = row_count;
...
function getProductWithBarcode(ev, el, userid, dsn2, dsn1, dsn3) {
    ...
    if (ev.keyCode == 13 || ev.type == 'change' || ev.key == '*') {
        ...
        if (Product.RECORDCOUNT != 0) {
            ...
            if (durum) {
                AddRow(Product.PRODUCT.PRODUCT_ID, Product.PRODUCT.STOCK_ID, 0, 1, parseFloat(Product.PRODUCT.PRICE).toString(), Product.PRODUCT.PRODUCT_NAME, Product.PRODUCT.TAX, Product.PRODUCT.DISCOUNT_RATE, 0, rafKodu, Product.PRODUCT.MONEY,parseFloat(Product.PRODUCT.PRICE).toString(),-6);
            }
            el.value='';
        } else {
...
function showData(el) {
    ...
    openBoxDraggable("index.cfm?fuseaction=objects.emptypopup_showdata_prt&rowid=" + row_id + "&p_name=" + p_name + "&tax=" + tax + "&disc=" + disc)
}
        ...
        $.ajax({
            ...
            success: function (retDat) {
                ...
                if (obj.ROW_ID.length > 0) {
                    UpdRow(obj.PID, obj.SID, 0, 1, obj.PRICE, obj.NAME, 20, 0, obj.ROW_ID);
                } else {
                    AddRow(obj.PID, obj.SID, 0, 1, obj.PRICE, obj.NAME, 20, 0, 1, '', 'TL', obj.PRICE,"-5");
                }
                closeBoxDraggable(modal_id)
            }
        })
...
function openHydrolic(id = "", row_id = "", tr = 0) {
    ...
    var price_catid = document.getElementById("PRICE_CATID").value;
    if (tr == 0) {
        openBoxDraggable("index.cfm?fuseaction=objects.emptypopup_partner_testpage&page=3&id=" + id + "&price_catid=" + price_catid + "&comp_id=" + comp_id + "&type=2&row_id=" + row_id)
    } else {
        openBoxDraggable("index.cfm?fuseaction=objects.emptypopup_partner_testpage&page=3&id=" + id + "&price_catid=" + price_catid + "&comp_id=" + comp_id + "&type=2&row_id=" + row_id)
    }
