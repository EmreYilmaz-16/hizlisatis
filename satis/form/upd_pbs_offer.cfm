...
WHERE OFFER_ID=#attributes.offer_id#
    ...
    <cfquery name="getOfferRow" datasource="#dsn3#">
        ...
        SELECT S.STOCK_CODE
	,CASE WHEN POR.IS_VIRTUAL = 1 THEN (SELECT PRODUCT_NAME FROM workcube_metosan_1.VIRTUAL_PRODUCTS_PRT WHERE VIRTUAL_PRODUCT_ID=POR.PRODUCT_ID) COLLATE SQL_Latin1_General_CP1_CI_AS  ELSE S.PRODUCT_NAME END AS PRODUCT_NAME
    ,CASE WHEN POR.IS_VIRTUAL = 1 THEN (SELECT PRODUCT_TYPE FROM workcube_metosan_1.VIRTUAL_PRODUCTS_PRT WHERE VIRTUAL_PRODUCT_ID=POR.PRODUCT_ID)   ELSE 0 END AS PRODUCT_TYPE
	,POR.PRODUCT_ID
	,POR.STOCK_ID
...
LEFT JOIN workcube_metosan_1.PRODUCT_BRANDS AS PB ON PB.BRAND_ID = CASE WHEN POR.IS_VIRTUAL=1 THEN 491 ELSE S.BRAND_ID END
