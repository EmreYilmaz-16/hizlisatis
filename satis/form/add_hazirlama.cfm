    ...
    <cf_box title="Ürün Hazırla">
        <cfif attributes.IS_SVK eq 1>
            <cfquery name="GETS" datasource="#DSN3#">
                ...
                SELECT AMOUNT
                ,WRK_ROW_RELATION_ID
                FROM #DSN#_#YEAR(NOW())-1#_#session.ep.COMPANY_ID#.SHIP_ROW AS S
                ) AS T
                        ...
                        <cfoutput query="getS">
                            <tr>
                                ...
                                <td style="width:%10">
                                    ...
                                    <input type="checkbox"  <cfif AMOUNT GTE QUANTITY>disabled checked</cfif> value="#currentrow#" name="roww" id="is_add#currentrow#"style="display:none">
                                </td>
                            </tr>
                            <cfif KM_P gte 1>
                                <cfquery name="getKp" datasource="#dsn3#">
...
<cfif isDefined("attributes.is_submit") >
    ...
    <cfquery name="AddSrow" datasource="#dsn2#">
        INSERT INTO STOCKS_ROW(STOCK_ID,PRODUCT_ID,UPD_ID,PROCESS_TYPE,STOCK_IN,STOCK_OUT,STORE,STORE_LOCATION,PROCESS_DATE,AMOUNT2,PROCESS_TIME)
        SELECT STOCK_ID,PRODUCT_ID,#TTT_IIID# AS UPD_ID,81 AS PROCESS_TYPE,AMOUNT STOCK_IN,0 STOCK_OUT,SHIP.DEPARTMENT_IN STORE,SHIP.LOCATION_IN STORE_LOCATION,GETDATE() PROCESS_DATE,1 AMOUNT2,GETDATE() PROCESS_TIME FROM workcube_metosan_2024_1.SHIP_ROW
LEFT JOIN workcube_metosan_2024_1.SHIP ON SHIP.SHIP_ID=SHIP_ROW.SHIP_ID
 WHERE SHIP_ROW.SHIP_ID=#TTT_IIID#
    </cfquery>
    <cfquery name="UP" datasource="#DSN2#">
        UPDATE SHIP SET DELIVER_EMP='Admin',IS_DELIVERED=1,DELIVER_EMP_ID=1 WHERE SHIP_ID=#TTT_IIID#
    </cfquery>
    <script>
