...
SELECT ORR.QUANTITY,SF.AMOUNT,S.PRODUCT_NAME,PP.SHELF_CODE,ORR.DELIVER_DEPT,ORR.DELIVER_LOCATION,S.STOCK_ID,SRR.SHIP_RESULT_ROW_ID,PP.PRODUCT_PLACE_ID FROM workcube_metosan_1.PRTOTM_SHIP_RESULT_ROW AS SRR
...
INNER JOIN workcube_metosan_1.ORDER_ROW AS ORR ON ORR.ORDER_ROW_ID=SRR.ORDER_ROW_ID
INNER JOIN workcube_metosan_1.PRTOTM_SHIP_RESULT AS PSR ON PSR.SHIP_RESULT_ID=SRR.SHIP_RESULT_ID
INNER JOIN workcube_metosan.COMPANY AS C ON C.COMPANY_ID=PSR.COMPANY_ID
INNER JOIN workcube_metosan_1.STOCKS AS S ON S.STOCK_ID=ORR.STOCK_ID
LEFT JOIN workcube_metosan_1.PRODUCT_PLACE_ROWS AS PPR ON PPR.STOCK_ID=ORR.STOCK_ID
INNER JOIN workcube_metosan_1.PRODUCT_PLACE AS PP ON PP.PRODUCT_PLACE_ID=PPR.PRODUCT_PLACE_ID
where PSR.SHIP_RESULT_ID=#attributes.SHIP_ID# AND ORR.DELIVER_DEPT=#attributes.DELIVER_DEPT# AND DELIVER_LOCATION=#attributes.DELIVER_LOCATION#
