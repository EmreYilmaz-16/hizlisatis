...
<cfquery name="Products" datasource="#dsn3#">
    ...
    LEFT JOIN (
	SELECT SUM(STOCK_IN-STOCK_OUT) AS BKY ,STOCK_ID,STORE,STORE_LOCATION FROM #DSN2#.STOCKS_ROW GROUP BY STOCK_ID,STORE,STORE_LOCATION
	) AS SR  ON SR.STOCK_ID=S.STOCK_ID
	LEFT JOIN workcube_metosan.DEPARTMENT AS D ON D.DEPARTMENT_ID=SR.STORE
	LEFT JOIN workcube_metosan.STOCKS_LOCATION AS SL ON SL.LOCATION_ID=SR.STORE_LOCATION AND SL.DEPARTMENT_ID=D.DEPARTMENT_ID
</cfif>
