...
var fatid=getParameterByName('cpid');
var elem=document.getElementsByClassName("detailHeadButton")
$(elem[0].children).append("<li class='dropdown' id='transformation'><a style='color:#04c76c' title='Firma Özel'onclick='pencereac(4,"+fatid+")'><i class='icon-book'></i></a></li>")
...
function pencereac(tip,idd){
    ...
    windowopen('index.cfm?fuseaction=sales.popup_add_ezgi_shipping&order_id='+idd,'wide');}else if(tip==2){
        ...
        windowopen('index.cfm?fuseaction=member.emptypopup_prtotm_comp_pass&cpid='+idd,'wide');
    }
}
