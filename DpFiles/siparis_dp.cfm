...
for(let i=0;i<window.basket.items.length;i++){
    ...
    str+=" INNER JOIN workcube_metosan.STOCKS_LOCATION AS SL ON SL.DEPARTMENT_ID = PP.STORE_ID	AND SL.LOCATION_ID = PP.LOCATION_ID INNER JOIN DEPARTMENT AS D ON D.DEPARTMENT_ID=SL.DEPARTMENT_ID AND D.BRANCH_ID=4 WHERE 1=1 AND PPR.STOCK_ID="+STOCK_ID;
