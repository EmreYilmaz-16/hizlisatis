...
FROM #DSN3#.PRTOTM_SHIP_RESULT AS PSR
LEFT JOIN #DSN3#.PRTOTM_SHIP_RESULT_ROW AS PSRR ON PSRR.SHIP_RESULT_ID = PSR.SHIP_RESULT_ID
...
LEFT JOIN (
	...
	FROM #DSN#_#YEAR(NOW())-1#_1.SHIP_ROW
	GROUP BY UNIQUE_RELATION_ID
...
LEFT JOIN (
	...
	GROUP BY UNIQUE_RELATION_ID,STOCK_ID
    UNION ALL
    SELECT SUM(AMOUNT) AS AMOUNT_H
		,UNIQUE_RELATION_ID
        ,STOCK_ID
	FROM #DSN#_#YEAR(NOW())-1#_1.STOCK_FIS_ROW
	GROUP BY UNIQUE_RELATION_ID,STOCK_ID

	) AS SIFR ON ORR.STOCK_ID=SIFR.STOCK_ID AND SIFR.UNIQUE_RELATION_ID = ORR.UNIQUE_RELATION_ID COLLATE SQL_Latin1_General_CP1_CI_AS
