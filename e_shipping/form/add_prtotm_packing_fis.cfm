...
<form name="add_package" method="post" action="<cfoutput>#request.self#?fuseaction=sales.emptypopup_add_PRTOTM_packing_fis&SHIP_ID=#attributes.ship_id#&type=#attributes.type#</cfoutput>">
	<tr class="color-list">
		<td colspan="6">
            <table cellpadding="2" cellspacing="1" width="99%">
                <tr>
                    ...
                    <td nowrap="nowrap"><input id="deliver_paper_no" class="boxtext" readonly name="deliver_paper_no" value="<cfoutput>#attributes.deliver_paper_no#</cfoutput>"></td>
              	</tr>
                <tr>
                    <td nowrap="nowrap"><b>Miktar</b></td>
                    <td nowrap="nowrap"><strong><input name="add_other_amount" type="text" value="<cfoutput>#attributes.add_other_amount#</cfoutput>" class="moneybox" style="width:60px;"></strong></td>
	...
	<cfoutput query="GET_SHIP_PACKAGE_LIST">
        ...
        <tr id="row#STOCK_ID#" height="20" onMouseOver="this.className='color-light';" onMouseOut="this.className='color-row';" class="color-row">
            ...
            <td>
            	<input type="text" id="packed_amount#STOCK_ID#" name="packed_amount#STOCK_ID#" readonly="yes" value="<cfif isdefined('control_amount#STOCK_ID#')>#Evaluate('control_amount#STOCK_ID#')#</cfif>" class="box"  style="width:25px;text-align:right;color:FF0000;">
            </td>
            <td>
                <input type="text" id="control_amount#STOCK_ID#" name="control_amount#STOCK_ID#" readonly="yes" value="0" class="box"  style="width:25px;text-align:right;color:FF0000;">
          	</td>
	...
	<tr class="color-list">
		<td colspan="6" align="right"><input type="button" value="Vazgeç" onclick="geri();" /> <input type="button" value="<cfif not get_detail_package_list.recordcount>Kaydet<cfelse>Güncelle</cfif>" onClick="if(confirm('Kaydetmek İstediğinizden Eminmisiniz?')) kontrol(); else return false;"></td><!--- <cf_workcube_buttons is_upd='0' is_delete='0' add_function='kontrol()'> --->
	</tr>
	</form>
</table>
...
{	
	...
	if(uzunluk > 14)
	{
		if(barcode.substr(0,1)=='j')//Bazı barkod okuyucular okuduktan sonra başına j harfi koyuyor kontrol için yapıldı
		{
			...
			barcode = barcode.substring(9,22);
		}
		else
		{
		lot_number = barcode.substring(0,8);
		barcode = barcode.substring(8,21);	
		}
	...
	if(list_find('<cfoutput>#product_barcode_list#</cfoutput>',barcode,','))
	{
		...
		var get_product = wrk_query(new_sql,'dsn1');
			...
			else
			{
				...
				{		
					if((document.getElementById('control_amount'+get_product.STOCK_ID).value*1) +(document.getElementById('packed_amount'+get_product.STOCK_ID).value*1) + (amount*1) > (document.getElementById('amount'+get_product.STOCK_ID).value*1))
					...
					if((document.getElementById('control_amount'+get_product.STOCK_ID).value*1)+(document.getElementById('packed_amount'+get_product.STOCK_ID).value*1) == (document.getElementById('amount'+get_product.STOCK_ID).value*1))
						document.getElementById('row'+get_product.STOCK_ID).style.display='none';
				}			
				else if(is_type==0)//silme ise	
					if((document.getElementById('control_amount'+get_product.STOCK_ID).value*1) == 0 || (document.getElementById('control_amount'+get_product.STOCK_ID).value*1) < 0)
						alert('Çıkan Ürünlerin Sayısı 0 dan küçük olamaz');
					else		
						document.getElementById('control_amount'+get_product.STOCK_ID).value = (document.getElementById('control_amount'+get_product.STOCK_ID).value*1)-(amount-1);
							if((document.getElementById('control_amount'+get_product.STOCK_ID).value*1) == (document.getElementById('amount'+get_product.STOCK_ID).value*1))
...
function geri()
{
	var sor = confirm('Kaydetmeden Çıkmak İstediğinizden Emin misiniz?');
	if(sor == true)
	history.go(-1);
	else
	return false;
