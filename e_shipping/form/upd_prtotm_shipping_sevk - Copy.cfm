...
<script language="javascript">
	...
	{//type sadece -1 olarak gelir,-1 geliyorsa hepsini seç demektir.
			...
			{
			window.open('<cfoutput>#request.self#</cfoutput>?fuseaction=sales.emptypopup_upd_PRTOTM_order_row&order_row_id_list='+order_row_id_list);
			}
	}
	function degisim()
	{
		sevk_emir = document.getElementById('select_name').value;
		window.open('<cfoutput>#request.self#?fuseaction=sales.emptypopup_upd_PRTOTM_sevk_emir&upd_id=#attributes.iid#&is_type=#attributes.is_type#</cfoutput>&sevk_emir='+sevk_emir);
	}
</script>