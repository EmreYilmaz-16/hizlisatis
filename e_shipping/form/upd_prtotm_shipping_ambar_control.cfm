                	...
                	CASE 
                    	WHEN 
                      	...
                      	THEN 
                        	S.PRODUCT_TREE_AMOUNT 
                      	ELSE 
                        	round(SIR.AMOUNT * EPS.PAKET_SAYISI,3) 
              		END 
                    	AS PAKET_SAYISI, 
                 	EPS.PAKET_ID, 
                    S.BARCOD, 
				...
				<!---<cfdump  var="#GET_SHIP_PACKAGE_LIST#">---->
					...
					<cfoutput query="GET_SHIP_PACKAGE_LIST">
						<cfquery name="getsLS" datasource="#dsn3#">
							SELECT SL.COMMENT,D.DEPARTMENT_HEAD FROM #dsn3#.PRODUCT_PLACE_ROWS AS PPR
LEFT JOIN #dsn3#.PRODUCT_PLACE AS PP ON PP.PRODUCT_PLACE_ID=PPR.PRODUCT_PLACE_ID
LEFT JOIN #dsn#.DEPARTMENT AS D ON D.DEPARTMENT_ID=PP.STORE_ID
LEFT JOIN #dsn#.STOCKS_LOCATION AS SL ON SL.LOCATION_ID=PP.LOCATION_ID  AND SL.DEPARTMENT_ID=D.DEPARTMENT_ID
...
<script language="javascript">
	...
	function kontrol(type)
	{
		...
		if(type==1)
		{
			...
			console.log(convert_list_amount);
			console.log(convert_list_price);
			var countx = (convert_list.match(/,/g) || []).length;
			console.log(countx)
			var i;
		var newlist="";
		var newlist2="";		
for (i = 1; i < countx+1; i++) {
 console.log(list_getat(convert_list,i,','));
 var ssss=list_getat(convert_list,i,',');
 var mmmm=list_getat(convert_list,i,',');
  var lll=0;
 if(ssss==mmmm){
	  newlist =newlist+list_getat(convert_list,i,',')+",";
	  lll=list_getat(convert_list_amount,i,',')+lll 
	 console.log(lll); }
 }
