	...
	if(list_find('<cfoutput>#product_barcode_list#</cfoutput>',barcode,','))
	{
		...
		var get_product = wrk_query(new_sql,'dsn1');
			...
			else
			{
				...
				{		
					<!---alert(parseFloat(filterNum(document.getElementById('packed_amount'+get_product.STOCK_ID).value,3))+ parseFloat(filterNum(document.getElementById('control_amount'+get_product.STOCK_ID).value,3)));--->
