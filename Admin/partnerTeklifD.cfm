...
workcube_metosan.getEmployeeWithId(PO.RECORD_MEMBER) as EMPO from #DSN3#.PBS_OFFER AS PO 
LEFT JOIN workcube_metosan.COMPANY AS C ON C.COMPANY_ID=PO.COMPANY_ID
LEFT JOIN #DSN3#.PBS_OFFER_TO_ORDER AS POTR ON POTR.OFFER_ID=PO.OFFER_ID
LEFT JOIN #DSN3#.ORDERS AS O ON O.ORDER_ID=POTR.ORDER_ID
LEFT JOIN (
SELECT DISTINCT ORDER_ID,PSR.DELIVER_PAPER_NO,PSR.SHIP_RESULT_ID FROM #DSN3#.PRTOTM_SHIP_RESULT_ROW AS PSRR
LEFT JOIN #DSN3#.PRTOTM_SHIP_RESULT AS PSR ON PSR.SHIP_RESULT_ID=PSRR.SHIP_RESULT_ID
) AS SRR ON SRR.ORDER_ID=O.ORDER_ID
LEFT JOIN #DSN3#.ORDERS_INVOICE AS OI ON OI.ORDER_ID=O.ORDER_ID AND OI.PERIOD_ID=3
LEFT JOIN workcube_metosan_2023_1.INVOICE AS I ON I.INVOICE_ID=OI.INVOICE_ID
LEFT JOIN #DSN3#.ORDERS_SHIP AS OS ON OS.ORDER_ID=O.ORDER_ID AND OI.PERIOD_ID=3
LEFT JOIN workcube_metosan_2023_1.SHIP AS S ON S.SHIP_ID=OS.SHIP_ID
LEFT JOIN (
SELECT PORR.RESULT_NO,VPO.V_P_ORDER_NO,PO.P_ORDER_NO,POR.OFFER_ID,PORR.PR_ORDER_ID,PORR.P_ORDER_ID FROM  #DSN3#.PBS_OFFER_ROW AS POR 
	LEFT JOIN #DSN3#.VIRTUAL_PRODUCTION_ORDERS AS VPO ON VPO.UNIQUE_RELATION_ID=POR.UNIQUE_RELATION_ID
	LEFT JOIN #DSN3#.VIRTUAL_PRODUCTION_ORDERS_RESULT AS VPOR ON VPOR.P_ORDER_ID=VPO.V_P_ORDER_ID
	LEFT JOIN #DSN3#.PRODUCTION_ORDER_RESULTS AS PORR ON PORR.PR_ORDER_ID=VPOR.REAL_RESULT_ID
	LEFT JOIN #DSN3#.PRODUCTION_ORDERS AS PO ON PO.P_ORDER_ID=PORR.P_ORDER_ID
