            ...
            <cfif IS_VIRTUAL EQ 1>
                <cfquery name="qProductTree1" datasource="#dsn3#">
                    SELECT 
                        ...
                        AMOUNT,
                        PB.BRAND_NAME,
                        CASE WHEN LEN((SELECT TOP 1  CAST(PRICE AS DECIMAL(18,2)) AS PRICE,CAST(DISCOUNT AS DECIMAL(18,2)) AS DISCOUNT,OTHER_MONEY FROM workcube_metosan_1.PROJECT_VIRTUAL_PRODUCTS_TREE_PRICES WHERE PBS_ROW_ID=VPT.PBS_ROW_ID AND IS_ACTIVE=1 FOR JSON AUTO)) >0 THEN (SELECT TOP 1  CAST(PRICE AS DECIMAL(18,2)) AS PRICE,CAST(DISCOUNT AS DECIMAL(18,2)) AS DISCOUNT,OTHER_MONEY FROM workcube_metosan_1.PROJECT_VIRTUAL_PRODUCTS_TREE_PRICES WHERE PBS_ROW_ID=VPT.PBS_ROW_ID AND IS_ACTIVE=1 FOR JSON AUTO) ELSE (SELECT '0' AS PRICE,'0' AS DISCOUNT,'TL' AS OTHER_MONEY FOR JSON PATH) END AS PRICEJSON,
                        2 AS SVY
