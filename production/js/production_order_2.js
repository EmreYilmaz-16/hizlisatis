...
function Hesapla(type) {
  var TotalPrice = 0;
  if (type == 1) {
    var questions = generalParamsSatis.Questions.filter(
      (p) => p.QUESTION_PRODUCT_TYPE == 1
    );
    for (let index = 0; index < hyd_basket_rows; index++) {
      var el =new Object();
      el.QUESTION_ID=index
...
function saveVirtual(Ptype, isFromVirtual) {
  if (Ptype == 1) {
  ...
  if (isFromVirtual == 1) {
    act = "/index.cfm?fuseaction=sales.emptypopup_update_virtual_product";
    mapForm.action = act;
  } else {
    act =
      "/index.cfm?fuseaction=production.popup_update_virtual_production_orders";
    mapForm.action = act;
  }
...
function addHydrolikRow(product, barcode) {
  ...
  if (product.RECORDCOUNT > 0) {
    ...
    if (generalParamsSatis.workingParams.IS_ADD_QUANTITY) {
      var ex = $(basket).find(
        "[data-stock_id='" + product.PRODUCT.STOCK_ID + "']"
      );
      if (ex.length > 0) {
