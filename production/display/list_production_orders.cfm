...
<cf_box title="Üretim Emirleri">
	...
	<CFSET TSLIS2TA=valueList(getEmpStation2.STATION_NAME)>
	<cfif getEmpStation.recordCount and len(getEmpStation.COMMENT)>	
		<cfquery name="getProductionOrders" datasource="#dsn3#">			
	SELECT * FROM (
SELECT VPO.V_P_ORDER_ID,VPO.V_P_ORDER_NO,VPO.QUANTITY  ,VPO.STOCK_ID,ISNULL(VPO_2.UNIQUE_RELATION_ID,VPO.UNIQUE_RELATION_ID) UNIQUE_RELATION_ID,PO.COMPANY_ID,C.NICKNAME,S.PRODUCT_NAME ,VPOR.REAL_RESULT_ID,
		...
		<cfif isDefined("getProductionOrders2")>
			<cfoutput query="getProductionOrders2">
				<tr>
					...
					<td>#PRODUCT_NAME#</td>
					<td>#listGetAt(TSLIS2TA,listFindNoCase(TSLIS2T,PRODUCT_TYPE))#</td>
					<td>#NICKNAME#</td>
					<td>#QUANTITY#</td>
				</tr>
			</cfoutput>
