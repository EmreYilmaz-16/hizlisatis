...
SELECT * FROM (
SELECT PO.P_ORDER_ID,PO.P_ORDER_NO,WS.STATION_NAME,S.PRODUCT_NAME,PO.SPECT_VAR_NAME,PO.START_DATE,PO.FINISH_DATE,PO.PROD_ORDER_STAGE,PO.LOT_NO,IS_STAGE,POR.ORDER_ROW_ID,ORR.DELIVER_DATE,PO.STATION_ID,O.ORDER_NUMBER,ISNULL(TKSS.AMOUNT,0) AMOUNT,PO.QUANTITY,C.NICKNAME FROM workcube_metosan_1.PRODUCTION_ORDERS  AS PO
...
LEFT JOIN workcube_metosan_1.STOCKS AS S ON S.STOCK_ID=PO.STOCK_ID
		...
		<cfif isDefined("getProductionOrders2")>
			<cfoutput query="getProductionOrders2">
				<tr>
					...
					<td>
						<a href="/index.cfm?fuseaction=prod.order&event=upd&upd=#P_ORDER_ID#" target="_blank">#P_ORDER_NO#</a>
					</td>
					<td>#PRODUCT_NAME#</td>
					<td>#NICKNAME#</td>
					<td>#QUANTITY#</td>
