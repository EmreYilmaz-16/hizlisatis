...
LEFT JOIN workcube_metosan_1.PROJECT_REAL_PRODUCTS_TREE_PRICES PEPS ON PEPS.PBS_ROW_ID=PT.PBS_ROW_ID AND IS_ACTIVE=1
 WHERE STOCK_ID=#arguments.sid# 
           <!----- select *,0 AS IS_VIRTUAL,(SELECT PROPERTY3 FROM PRODUCT_INFO_PLUS WHERE PRODUCT_ID=PRODUCT_TREE.PRODUCT_ID AND PRO_INFO_ID=2) AS DISPLAY_NAME,PRICE_PBS AS PRICE,OTHER_MONEY_PBS AS MONEY,DISCOUNT_PBS AS DISCOUNT from PRODUCT_TREE WHERE STOCK_ID=----->
    </cfif>
            </cfquery>       
