    ...
    <cffunction name="getTrees">
        ...
        <cfquery name="getTree" datasource="#dsn3#">
          ...
          <!----  SELECT *,STOCK_ID AS RELATED_ID,VPT_ID AS PRODUCT_TREE_ID,PRICE,DISCOUNT,MONEY FROM VIRTUAL_PRODUCT_TREE_PRT WHERE VP_ID=#arguments.pid# AND PRODUCT_ID <>0------>
            ...
            SELECT VPT_ID,VP_ID,PRODUCT_ID,STOCK_ID,VPT.AMOUNT,QUESTION_ID,IS_VIRTUAL,DISPLAY_NAME,PEPS.PRICE,PEPS.DISCOUNT,PEPS.OTHER_MONEY AS MONEY ,PEPS.MAIN_PRODUCT_ID,STOCK_ID AS RELATED_ID,VPT_ID AS PRODUCT_TREE_ID 
            FROM workcube_metosan_1.VIRTUAL_PRODUCT_TREE_PRT  AS VPT
            LEFT JOIN workcube_metosan_1.PROJECT_VIRTUAL_PRODUCTS_TREE_PRICES PEPS ON PEPS.PRODUCT_TREE_ID=VPT.VPT_ID AND IS_ACTIVE=1

WHERE VP_ID=#arguments.pid# AND PRODUCT_ID <>0

            <cfelse>
                select *,0 AS IS_VIRTUAL,(SELECT PROPERTY3 FROM workcube_metosan_1.PRODUCT_INFO_PLUS WHERE PRODUCT_ID=PT.PRODUCT_ID AND PRO_INFO_ID=2) AS DISPLAY_NAME,PEPS.PRICE AS PRICE,PEPS.OTHER_MONEY AS MONEY,PEPS.DISCOUNT AS DISCOUNT from workcube_metosan_1.PRODUCT_TREE
AS PT
