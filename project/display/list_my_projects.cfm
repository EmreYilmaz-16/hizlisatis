...
workcube_metosan.getEmployeeWithId(PROJECT_EMP_ID) AS YONETICI,SETUP_PRIORITY.PRIORITY,
SETUP_PRIORITY.COLOR,COMPANY.NICKNAME from workcube_metosan.PRO_PROJECTS
...
LEFT JOIN workcube_metosan.SETUP_MAIN_PROCESS_CAT AS SMP ON SMP.MAIN_PROCESS_CAT_ID=PRO_PROJECTS.PROCESS_CAT
LEFT JOIN workcube_metosan.PROCESS_TYPE_ROWS AS PTP ON PTP.PROCESS_ROW_ID=PRO_PROJECTS.PRO_CURRENCY_ID
INNER JOIN workcube_metosan.COMPANY ON COMPANY.COMPANY_ID=PRO_PROJECTS.COMPANY_ID where PROJECT_EMP_ID =#session.ep.userid# 
</cfquery>
