...
<cfif attributes.act eq "list_my">
    <cf_box title="Notlar">       
    <cfquery name="getNotes" datasource="#dsn#">
        select NOTE_HEAD,NOTE_ID,NOTE_BODY,IS_WARNING,ACTION_SECTION,ACTION_ID,workcube_metosan.getEmployeeWithId(RECORD_EMP) RECORD_EMP,RECORD_DATE from workcube_metosan.NOTES WHERE ACTION_SECTION='PROJECT_ID' AND RECORD_EMP=#session.ep.userid#
    </cfquery>
    <div style="clear:both"></div>
    <div class="row">
        <cfoutput query="getNotes">
