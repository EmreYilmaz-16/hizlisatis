<cfif attributes.tool_type eq "alternativeQuestion">
   ...
   <cfquery name="getAlternativeQuestions" datasource="#dsn#">
    ...
    <select name="aquestion" id="aquestion" onchange="setAQuestions(<cfoutput>#attributes.idb#</cfoutput>,this.value)">
        <option value="">Alternatif Sorusu</option>
        <cfoutput query="getAlternativeQuestions">
            <option value="#QUESTION_ID#">#QUESTION_NAME#</option>
        </cfoutput>
    </select>
<button type="button" class="btn btn-danger" onclick="closeBoxDraggable('<cfoutput>#attributes.modal_id#</cfoutput>')"></button>
</cf_box>
