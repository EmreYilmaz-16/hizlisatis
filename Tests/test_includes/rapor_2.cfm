    ...
    <cfquery name="getpp" datasource="#dsn#">
        ...
        SELECT DISTINCT * FROM (
          ...
          SELECT PROJECT_ID,PROJECT_NUMBER,PROJECT_HEAD FROM workcube_metosan.PRO_PROJECTS                         
             WHERE PROJECT_ID IN (
                 SELECT PROJECT_ID FROM #dsn2#.CARI_ROWS WHERE FROM_CMP_ID=#getc.COMPANY_ID# OR TO_CMP_ID=#getc.COMPANY_ID#
             )
             UNION ALL
...
CASE WHEN TO_CMP_ID IS NOT NULL THEN SUM(ACTION_VALUE) END AS AR
,PROJECT_ID
FROM #dsn2#.CARI_ROWS WHERE (FROM_CMP_ID=#getc.COMPANY_ID# OR TO_CMP_ID=#getc.COMPANY_ID# )and ISNULL(PROJECT_ID,0)=TF.PROJECT_ID
...
COMPANY_ID,ISNULL(PROJECT_ID,0) PROJECT_ID,ISNULL(SC,0) SC
 FROM (
SELECT * FROM #dsn2#.CEK_RISKI
UNION ALL
SELECT * FROM #dsn2#.SENET_RISKI
